...
local function unpack(s)
    ...
    log.debug("mqtt.unpack", #s, string.toHex(string.sub(s, 47, 60)))
    mqtt_test=s--string.toHex(string.sub(s, 46, 60))
...
function mqttc:publish(topic, payload, qos, retain)
    ...
    if not self:write1(packPUBLISH(0, qos, retain, qos > 0 and self.getNextPacketId() or 0, topic, payload)) then
...
function mqttc:publish0(topic, payload, qos, retain)
    if not self.connected then
        log.info("mqtt.client:publish", "not connected")
        return false
    end
    ...
    qos = qos or 0
    retain = retain or 0
    local mun = string.format("%#X",#payload)
    local numbers = string.format("%#X",#payload+8)
    local a=0
    log.info("mqtt.client:publish", payload,#payload,mun)
    if #payload<16 then
        a = str2hex("30"..(#payload+2).."000324647003000"..mun..hex2str(payload))
    elseif #payload<20 then
        a= str2hex("30"..(#payload+2).."00032464700300"..mun..hex2str(payload))
    else
        a = str2hex("30"..numbers.."00032464700300"..mun..hex2str(payload))
    end
    if not self:write1(a)then
        log.info("mqtt.client:publish", "socket send failed")
        return false
    end
